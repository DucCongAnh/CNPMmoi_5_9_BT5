...
const AdvancedSearch = ({ onSearchResults, onFilterChange, isSidebar = false }) => {
  ...
  const [showFilters, setShowFilters] = useState(isSidebar);
  const [loading, setLoading] = useState(false);
  const [searchResults, setSearchResults] = useState(null);

  useEffect(() => {
    setShowFilters(isSidebar);
  }, [isSidebar]);
  ...
  return (
    <div className={`advanced-search ${isSidebar ? 'as-sidebar' : ''}`}>
      {/* Search Input */}
      ...
      {/* Advanced Filters */}
      {showFilters && (
        <div className={`advanced-filters ${isSidebar ? 'vertical' : ''}`}>
          <div className={`filters-grid ${isSidebar ? 'grid-1' : ''}`}>
            {/* Category Filter */}
